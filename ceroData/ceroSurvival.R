survPlotterCero <- function(variable, expTable,histParams, clinicalTable, Type) {
  library(survival)
  library(survminer)
  if (Type == "Lymph Node") {
    redCclinicalTable <- clinicalTable[clinicalTable$SampleType == "LN",]
  } else if (Type == "Cutaneous Metastasis") {
    redCclinicalTable <- clinicalTable[clinicalTable$SampleType == "CM",]
  } else if (Type == "Primary") {
    redCclinicalTable <- clinicalTable[clinicalTable$SampleType == "PT",]
  }
  ...
  usedTable <- data.frame("ID" = reducedClin2$Sample_ID,
                          "Time" = reducedClin2$`OS(months)`, 
                          "Event" = reducedClin2$Live,
                          stringsAsFactors = T)
  ...
  colnames(usedTable) <- c("ID", "Time", "Event", "Value")
  ...
  usedTable$Time <- as.numeric(usedTable$Time)
